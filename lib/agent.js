...
export async function runTask(task, { mode = 'root', onStep = () => {}, maxSteps } = {}) {
  ...
  let step = 0;
  let nudges = 0;
  // a weak model needs several pushes to finish a multi-action chain (open → type →
  // send → verify). Generous, but capped so a truly-stuck run still terminates.
  const MAX_NUDGES = parseInt(process.env.MICHI_MAX_NUDGES || '8', 10);
  while (step < limit) {
    step++;
    ...
    // No tool calls => Michi may be done... or STALLED, or LYING. Weak local
    // models (9B) do three bad things at the end of a chain: (a) emit an empty turn
    // and quit; (b) claim success ("Message sent!") that NEVER HAPPENED to escape
    // the nudges; (c) stop a few actions in. So before accepting "done":
    //   - if it produced NO real text → it stalled → nudge to continue.
    //   - if it CLAIMS completion but recent history shows no terminal action
    //     (a movicom result with did:sent / did:submitted), distrust it → nudge to
    //     actually do it and VERIFY. (scar 2026-06-02: qwen said "sent" 4x, sent 0.)
    if (!reply.toolCalls || reply.toolCalls.length === 0) {
      const trivialText = !reply.text || reply.text.trim().length < 3;
      const claimsDone = /\b(sent|done|complete|finished|success|enviad|listo|hecho)\b/i.test(reply.text || '');
      // did a terminal action actually happen in the last few tool results?
      const recentToolText = messages.slice(-6).filter(m => m.role === 'tool').map(m => String(m.content)).join(' ');
      // a TERMINAL action = actually sent/submitted. typing/opening is NOT done —
      // scar 2026-06-02: qwen typed the message correctly then stopped one step
      // short of `ui do send`, and "did:typed" wrongly counted as completion.
      const reallyActed = /"did":"(sent|submitted)|"sent"|submitted \(enter\)/.test(recentToolText);
      // extra push: if it just typed but didn't send, say so explicitly
      const typedNotSent = /"did":"typed/.test(recentToolText) && !reallyActed;
      const unverifiedClaim = (claimsDone || typedNotSent) && !reallyActed;

      if ((trivialText || unverifiedClaim) && nudges < MAX_NUDGES) {
        nudges++;
        await emit({ kind: 'nudge', step, n: nudges, why: unverifiedClaim ? 'unverified-claim' : 'stalled' });
        messages.push({ role: 'assistant', content: reply.text || '' });
        messages.push({ role: 'user', content: unverifiedClaim
          ? "You haven't actually completed the task yet — I don't see the action in the " +
            "phone's results. Do NOT claim it's done. Take the next concrete movicom step " +
            "now (read the screen with `ui frame`, then `ui do <n>` or `ui do type/send`), " +
            "and only report success after a movicom result confirms it (e.g. did:sent, or " +
            "the text visible in `ui frame`)."
          : "Continue the task. Call the movicom tool with the next command — `ui frame` to " +
            "read the screen, or `ui do <n>` / `ui do type \"…\"` / `ui do send` to act. " +
            "Only say you're finished after the phone confirms it." });
        continue;
      }
      messages.push({ role: 'assistant', content: reply.text || '' });
