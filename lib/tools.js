...
import { ctx } from './context-tags.js';
...
export const TOOL_SCHEMAS = [
  {
    ...
    function: {
      name: 'movicom',
      description:
        "Michi's body: drive a real Android phone. Pass the movicom command WITHOUT " +
        "the leading 'movicom' word. Output is always one JSON value.\n" +
        'THE LOOP — read the screen as a FRAME, then act by number:\n' +
        '  ui frame          → {app, read:[content], do:["1 type <text>","2 send",\n' +
        '                       "3 up","4 down","5 back","6 home","7 open: <row>",...]}\n' +
        '  ui do <n>         → run action #n; RETURNS the next frame (no separate read)\n' +
        '  ui do <n> "text"  → run action #n with text (e.g. type into an input)\n' +
        '  ui do <verb> [t]  → verb mode: type / send / up / down / back / home / more\n' +
        'Read `read` for content, pick a number/verb from `do`. Never reason about pixels.\n' +
        'Getting around:\n' +
        '  doctor                  — device + current app (start here)\n' +
        '  app fresh <name>        — open an app at a clean start (e.g. app fresh whatsapp)\n' +
        '  app store <name>        — open its Play Store page (then ui do the Install)\n' +
        '  contacts find <q> | notif list — talk to the OS, not the glass, when you can\n' +
        'Example — send a WhatsApp: `app fresh whatsapp` → `ui frame` → `ui do 7` (open the\n' +
        'chat) → `ui do 1 "hola"` (type) → `ui do send`.',
      parameters: {
        type: 'object',
        properties: {
          command: { type: 'string', description: 'movicom args, e.g. `ui frame` or `ui do 1 "hola"`' },
        },
        required: ['command'],
      },
    },
...
async function execMovicom(command) {
  ...
  if (!out) return JSON.stringify({ ok: true, note: 'no output' });
  // A movicom SCREEN read (a frame, or an action result that folds in the screen)
  // describes the screen RIGHT NOW and is superseded the moment Michi acts again.
  // Tag it `supersede=screen` so the engine keeps only the LATEST screen in context
  // and collapses older ones (Andy 2026-06-02: the <context:…> protocol). Non-screen
  // results (opened/saved/notif list/…) are durable and left untagged.
  if (/"do":\[|"screen":\{|"actions":\[|"read":\[/.test(out)) return ctx.supersede(out, 'screen');
  return out;
}
