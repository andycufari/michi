...
async function chatOpenAI(c, messages, tools) {
  ...
  const body = {
    ...
    temperature: 0.3,
    // Cap output so a small-context local model (LM Studio reserves max_tokens out
    // of the TOTAL window) keeps room for input. Phone steps are short — a tool
    // call + a sentence. Override with MICHI_MAX_TOKENS if a task needs long prose.
    max_tokens: parseInt(process.env.MICHI_MAX_TOKENS || '1024', 10),
  };
